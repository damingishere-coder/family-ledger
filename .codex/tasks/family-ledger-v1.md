...
## 2026-09-01 FamilyLedger Logo 设计与接入

### 背景与目标

当前侧栏品牌由 Lucide 的房屋与家庭图标临时叠加，浏览器入口没有 favicon。用户要求先查看多套正式 Logo，再把选定方案统一用于侧栏和网页选项卡。

### 允许修改范围

- `docs/brand/` 中的四套 SVG 候选、组合字标和对比预览。
- 用户选定后的 `frontend/public/brand/` 正式资源、`Layout.tsx`、`styles.css` 与 `index.html`。
- 本任务说明，以及与静态资源接入直接相关的测试或验证脚本。

### 禁止修改范围

- 后端 API、数据库、金额计算、盘点流程和公开 TypeScript 数据类型。
- 联网字体、图片服务、第三方 UI 或图像处理运行时依赖。
- 未经用户选定即把任一候选接入正式页面，或自动合并 PR。

### 已确定实现要求

- 先制作 A「檐下家人」、B「账本之家」、C「安心家徽」、D「FL 屋形字母」四套代码原生 SVG。
- 每套包含方形图标和“家底 · FamilyLedger”横版组合，主蓝 `#0A63F6`、深蓝 `#0B1739`、透明画布、无渐变。
- 生成单张编号对比预览，并检查 16、32、42、180px 尺寸下的识别度。
- 用户已选定 B「账本之家」；将其生成正式 favicon、PNG fallback 与 Apple Touch Icon，并将侧栏品牌改为可返回概览的链接。
- Logo 只在侧栏常驻，不在各页面标题区重复展示；页面标题统一为“家底 · FamilyLedger”。
